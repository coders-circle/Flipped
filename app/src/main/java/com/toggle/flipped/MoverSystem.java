            ...
            if (m.moving) {
                ...
                if (m.type == Mover.Type.LINEAR) {
                    if (Math.abs(t.x - fx) < 8 && Math.abs(t.y - fy) < 8) {
                        ...
                        if (m.cameraFocus && bot.cameraPos != null && bot.cameraPos.x == fx && bot.cameraPos.y == fy)
                            bot.cameraPos = null;
                        b.body.setLinearVelocity(new Vector2(0, 0));
                        m.moving = false;

                        if (!m.canReverse || m.reverse)
                            m.cameraFocus = false;
                    } else {
                        Bot bot = m.player.get(Bot.class);
                        if (m.cameraFocus)
                    ...
                    if (Math.abs(t.angle - fangle) < 3) {
                        Bot bot = m.player.get(Bot.class);
                        if (m.cameraFocus && bot.cameraPos != null && bot.cameraPos.x == t.x && bot.cameraPos.y == t.y)
                            bot.cameraPos = null;
